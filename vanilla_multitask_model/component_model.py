...
class Encoder(nn.Module):
    def __init__(self, config, testing=False):
        ...
        if not self.config['hope_classification_heads'] and not self.config['gms_classification_heads']:
            print('One set of scores has to be on')
            exit(1)
...
class ClassificationHeads(nn.Module):
    ...
    def forward(self, x):
        ...
        score = self.classification_head(x)
        return score
...
class Vanilla_Multitask_Model(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.config['hope_classification_heads'] and not self.config['gms_classification_heads']:
            return (meatus_pos_score, meatus_shape_score, glans_shape_score, penile_skin_shape_score, torsion_score)
