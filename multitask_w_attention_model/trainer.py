...
class Trainer:
    ...
    def update_metrics(self, y_pred, y, loss):
        ...
        y_pred = torch.max(torch.softmax(torch.stack(y_pred).squeeze(2).detach().cpu(), dim=-1), dim=-1)[1]
        y = y.detach().cpu()
    ...
    def train_epoch(self, data_loader):
        ...
        for i, batch in enumerate(tqdm(data_loader)):
            ...
            names, X, y, mask = batch[0], batch[1], batch[2], batch[3]
            ...
            for j, bodypart in enumerate(self.config['anatomy_part']):
                ...
                # TODO: 6th nov night -- all works, just change names to CE instead of MSE
                ce_loss = self.criterion[1](y_pred, torch.flatten(y[j]))
                attention_loss = self.criterion[0](X, torch.flatten(y[j]), mask)  # should be y instead of y_pred
                ...
                wandb.log({f'{bodypart}_batch_loss_ce': ce_loss.item(), 'step': self.global_step})
                wandb.log({f'{bodypart}_batch_loss_attn': attention_loss.item(), 'step': self.global_step})
                
                # save train preds
                softmax_y_pred = torch.softmax(y_pred, dim=-1)
                for k, name in enumerate(names):
                    ...
                    label = y[j][k].item()
                    self.train_preds[f'{name}-{bodypart}'].append(softmax_y_pred[k][label].item())
                    self.train_labels[f'{name}-{bodypart}'].append(label)
    ...
    def validate_epoch(self, data_loader, data_mode='val'):
        ...
        for i, batch in enumerate(tqdm(data_loader)):
            ...
            with torch.no_grad():
                ...
                for j, bodypart in enumerate(self.config['anatomy_part']):
                    # calculate y_pred
                    y_preds.append(self.models[bodypart](X))

            for j, bodypart in enumerate(self.config['anatomy_part']):

                ce_loss = self.criterion[1](y_preds[j], torch.flatten(y[j]))
                attention_loss = self.criterion[0](X, torch.flatten(y[j]), mask)
                
                loss = ce_loss + attention_loss
                losses.append(loss.item())

                # save val preds
                softmax_y_pred = torch.softmax(y_preds[j], dim=-1)
                for k, name in enumerate(names):
                    ...
                    label = y[j][k].item()
                    self.val_preds[f'{data_mode}_{name}-{bodypart}'].append(softmax_y_pred[k][label].item())
                    self.val_labels[f'{data_mode}_{name}-{bodypart}'].append(label)
