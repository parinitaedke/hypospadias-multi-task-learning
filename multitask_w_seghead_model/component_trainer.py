...
class Trainer:
    ...
    def train_epoch(self, data_loader, include_mask=True, overlap_mask_penalty=False):
        ...
        for i, batch in enumerate(tqdm(data_loader)):
            ...
            if self.config['hope_classification_heads']:
                y_score_preds, y_hope_score_preds = self.models['classification_heads'](x)
            else:
                y_score_preds = self.models['classification_heads'](x)
            ...
            # Classification heads + Encoder backwards pass update
            self.models['classification_heads'].zero_grad()
            self.models['encoder'].zero_grad()
            total_classification_loss.backward() # retain_graph=True
            self.optimizers['encoder'].step()
            self.optimizers['classification_heads'].step()
            self.global_step += 2
        ...
        for epoch in range(self.config['num_epochs']):
            ...
            if self.config['save_weights']:
                if (epoch%5 == 0) or (self.config['num_epochs'] == epoch+1):
                    ...
                    for component in ['encoder', 'decoder', 'classification_heads']:
                        torch.save(self.models[component].state_dict(), f"{self.model_checkpoints_dir}/model-{self.global_epoch}")
